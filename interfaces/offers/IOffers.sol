    ...
    ILendingStructs
{
    ...
    /// @param offerHash The hash of all parameters in an offer
    function getFloorOfferCount(bytes32 offerHash)
        external
        view
    ...
    function getSigFloorOfferCount(bytes memory signature)
        external
        view
        returns (uint64 count);
    ...
    /// @param signature The signature of the offer
    function markSignatureUsed(Offer memory offer, bytes memory signature)
        external;
